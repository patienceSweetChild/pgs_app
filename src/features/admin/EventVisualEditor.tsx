...
function str(draft: EventDraft, key: string): string {
  const v = draft[key];
...
function CanvaImage({
  ...
  async function onFile(file: File | undefined) {
    ...
    try {
      ...
      const uploaded = await uploadMediaAsset(supabase, file, "events", {
        imagesOnly: true,
      });
...
function EditableProgramCard({
  ...
  const data = compact
    ? eventToProgramCompact(draft)
    : eventToProgramFull(draft);
    ...
    <div className="pgs-cards pgs-admin-visual__canva-card">
      <article className={`cardbox${compact ? " cardbox--compact" : ""}`}>
        ...
        <div className="cardbox-middle">
          ...
          {!compact ? (
            <div className="cardbox-highlight">
              ...
              <CanvaText
                ...
                onCommit={(v) => patchDraft(draft, onChange, { "summary": v })}
                multiline
          ...
          {compact ? (
            <div className="cardbox-detail-stack">
              <div className="cardbox-detail-label">Host</div>
              <CanvaText
                as="div"
                className="cardbox-detail-value"
                value={str(draft, "host")}
                onCommit={(v) => patchDraft(draft, onChange, { "host": v })}
                placeholder="Host"
              />
            </div>
          ) : (
...
function EditablePromoCard({
  ...
  const data = eventToPromoCard(draft);
  return (
        ...
        <div className="sop-image-wrapper">
          ...
          <div className="sop-start-free">
            <CanvaText
              ...
              onCommit={(v) => patchDraft(draft, onChange, { "top_label": v })}
...
function EditableInternshipCard({
  ...
  const data = eventToInternshipCard(draft);
  return (
        ...
        <div className="pgs-session-card__body">
          <div>
            ...
            <CanvaText
              ...
              onCommit={(v) => patchDraft(draft, onChange, { "who_is_it_for": v })}
              multiline
          ...
          <div className="pgs-session-card__topics">
            ...
            <CanvaText
              ...
              onCommit={(v) => patchDraft(draft, onChange, { "session_topics": v })}
              multiline
        ...
        <div className="d-flex gap-3 mt-2 flex-wrap">
          ...
          <div className="content-p" style={{ maxWidth: 280 }}>
            ...
            <CanvaText
              ...
              onCommit={(v) => patchDraft(draft, onChange, { "description": v })}
              multiline
              placeholder="Description"
            />
          </div>
...
export function EventVisualEditor({ draft, onChange }: Props) {
  const [activeVisual, setActiveVisual] = useState<EventVisualKey | "all">(
    () => {
      const enabled = parseCardSurfaces(draft);
  ...
  const visibleKeys = useMemo(
    () => (activeVisual === "all" ? [...EVENT_VISUAL_KEYS] : [activeVisual]),
    [activeVisual],
  );
    ...
    <div className="pgs-admin-visual">
      ...
      <div className="pgs-admin-visual__summary">
        <AdminRichTextField
          ...
          value={draft.summary == null ? "" : String(draft.summary)}
          onChange={(next) => onChange({ ...draft, summary: next })}
          rows={4}
        />
        <label>
          Location (cards)
